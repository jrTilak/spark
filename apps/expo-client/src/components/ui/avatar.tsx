...
const Avatar = React.forwardRef<AvatarPrimitive.RootRef, AvatarPrimitive.RootProps>(
  ({ className, ...props }, ref) => (
    <AvatarPrimitive.Root
      ref={ref}
      className={cn('relative flex h-10 w-10 shrink-0 overflow-hidden rounded-full bg-muted/30', className)}
      {...props}
    />
  )
);
    ...
    >
      {
        typeof children === "string" && children.length > 2 && initials ?
          <Text>{children.split(' ').map((word) => word[0]?.toUpperCase()).slice(0, 2).join('')}</Text>
          : children
      }
    </AvatarPrimitive.Fallback>
  )
